...
object PartOne {
	...
	def main(args: Array[String]) = {
		...
		var one = new Stack[Char]()
		one ++= List('V', 'R', 'H', 'B', 'G', 'D', 'W')
		var two = new Stack[Char]()
		two ++= List('F', 'R', 'C', 'G', 'N', 'J')
		var three = new Stack[Char]()
		three ++= List('J', 'N', 'D', 'H', 'F', 'S', 'L')
		var four = new Stack[Char]()
		four ++= List('V', 'S', 'D', 'J')
		var five = new Stack[Char]()
		five ++= List('V', 'N', 'W', 'Q', 'R', 'D', 'H', 'S')
		var six = new Stack[Char]()
		six ++= List('M', 'C', 'H', 'G', 'P')
		var seven = new Stack[Char]()
		seven ++= List('C', 'H', 'Z', 'L', 'G', 'B', 'J', 'F')
		var eight = new Stack[Char]()
		eight ++= List('R', 'J', 'S')
		var nine = new Stack[Char]()
		nine ++= List('M', 'V', 'N', 'B', 'R', 'S', 'G', 'L')
		...
		var registeredStacks = LinkedHashMap[Int, Stack[Char]](
			1 -> one,
			2 -> two,
			3 -> three,
			4 -> four,
		...
		// var test = new Stack[Char]()
		// test ++= List('T', 'I', 'M')
		// val zaba = test.take(2)
		// var test2 = new Stack[Char]()
		// test2.pushAll(zaba)
		// println(test2)
		...
		for (line <- Source.fromFile(inputPath).getLines) {
			...
			registeredStacks(target) = registeredStacks(target).pushAll(x)
